...
class BigQueryRetrievalJob(RetrievalJob):
    ...
    def to_bq(self, dataset, table, expire_in_days=1):
        """
        Runs and uploads the the final historical retrieval directly to BigQuery

        Args:
            dataset: BigQuery dataset name
            table: BigQuery table name
            expire_in_days: the number of days from present to set the expiration timestamp on the table (default = 1)
        """
        # other options at: https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#table_option_list
        query = f"""CREATE OR REPLACE TABLE `{self.client.project}.{dataset}.{table}`
                    OPTIONS
                    (
                        expiration_timestamp=TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL {expire_in_days} DAY),
                        description="Autogenerated table from BigQueryRetrievalJob.save_to_bigquery()"
